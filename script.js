...
/* =========================================================
   ...
   ---------------------------------------------------------
   Sends automatic WhatsApp order confirmations to customers
   ========================================================= */
var ULTRAMSG_INSTANCE_ID = "190046";
var ULTRAMSG_API_TOKEN = "64g03dgxi2d8a8qf";
var ULTRAMSG_BUSINESS_NUMBER = "201000700965"; // Your WhatsApp Business number
...
function submitOrder() {
    ...
    var itemsList = items.map(function(d) { return d.qty + 'x ' + d.product.name + (d.variant ? ' (' + d.variant + ')' : '') + ' — EGP ' + d.lineTotal.toLocaleString(); }).join('\n');
...
/* =========================================================
   SEND WHATSAPP CONFIRMATION VIA ULTRAMSG
   ========================================================= */
function sendWhatsAppConfirmation(params) {
    var customerPhone = params.phone.replace(/\D/g, ''); // Remove non-digits
    if (customerPhone.length === 10) {
        customerPhone = '20' + customerPhone; // Add country code if missing
    }
    if (!customerPhone.startsWith('20')) {
...
For support, contact us anytime.`;
    ...
    fetch(url, {
        ...
        body: 'token=' + ULTRAMSG_API_TOKEN + '&to=' + customerPhone + '&body=' + encodeURIComponent(message)
    })
    .then(function(response) {
        console.log('WhatsApp message sent successfully');
    })
    .catch(function(error) {
        console.log('WhatsApp message failed (non-critical):', error);
    });
}
